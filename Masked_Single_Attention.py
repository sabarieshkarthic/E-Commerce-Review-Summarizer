...
class MaskedSingleAttention:
    ...
    def intializeMaskedAttention(self,X):
        row,no_of_neurons=X.shape
        self.X=X
        K=np.dot(self.X,self.weight_K)
        Q=np.dot(self.X,self.weight_Q)
        V=np.dot(self.X,self.weight_V)
        result_att=self.Masked_Softmax(K,Q,V)
        return result_att
    ...
    def backpropagate(self,d_output):
        ...
        for i in range(self.d_QKT.shape[0]):
            self.d_QKT_scaled[i,i+1:]=0.0
       
        self.d_Q=np.dot(self.d_QKT,self.K)
        self.d_K=np.dot(self.d_QKT.T,self.Q)
       
        self.d_weight_Q=np.dot(self.X.T,self.d_Q)
        self.d_weight_K=np.dot(self.X.T,self.d_K)
        self.d_weight_V=np.dot(self.X.T,self.d_V)
       
        self.d_X=(np.dot(self.d_Q,self.weight_Q.T)+np.dot(self.d_V,self.weight_V.T)+np.dot(self.d_K,self.weight_K.T))
